...
import { D, getAllCategories, isCustomId } from "@/lib/data";
import {
  CACHE_KEYS,
  saveJson,
  loadJson,
...
export async function scanYoutube(onProgress, opts = {}) {
  const { scope = "blue", force = false } = opts;
  onProgress?.({ stage: "youtube", status: "start", message: `YouTube 스캔 중 (scope=${scope}${force ? ", force" : ""})…` });
  try {
    const url = `/api/youtube-scan?scope=${encodeURIComponent(scope)}${force ? "&force=true" : ""}`;
    const data = await fetchJson(url);
    saveJson(CACHE_KEYS.youtube, data);
    const cacheHitsMsg = data.cacheHits ? ` · 캐시 적중 ${data.cacheHits}` : "";
    onProgress?.({
      stage: "youtube",
      status: "done",
      message: `${data.results.length}개 (${scope}) · 쿼터 +${data.quotaUsed}${cacheHitsMsg}`,
    });
    return data;
  } catch (e) {
...
export async function scanClaudeAnalysis(opts = {}) {
  ...
  // 커스텀 카테고리 포함 — Claude 분석은 단건 라우트라 커스텀도 처리 가능
  const allCats = getAllCategories();
  const targets =
    scope === "missing"
      ? allCats.filter((c) => !existing[String(c.id)])
      : allCats;
  ...
  for (let i = 0; i < targets.length; i += chunkSize) {
    ...
    const settled = await Promise.allSettled(
      chunk.map(async (cat) => {
        const payload = {
          categoryId: cat.id,
          // 커스텀 카테고리(1000+)는 서버가 D 에서 못 찾으므로 category 객체 동봉
          category: isCustomId(cat.id) ? cat : undefined,
  ...
  if (opts.youtube) {
    try {
      summary.youtube = await scanYoutube(onProgress, {
        scope: opts.youtubeScope || "blue",
        force: !!opts.youtubeForce,
      });
    } catch (e) {
