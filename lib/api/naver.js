...
// ─── 브랜드/셀러 단위 리뷰 체크 (파트너 발견용) ────────────────────────────────
//
// 간이 감성 분석: 제목·설명에서 긍정/부정 키워드 카운트로 sentimentScore 계산.
// 정확한 감성 분석은 Claude 가 별도로 수행. 여기는 1차 스크리닝용 빠른 신호.

const POSITIVE_KEYWORDS = ["추천", "만족", "좋", "대박", "최고", "합리", "가성비", "강추", "굿"];
const NEGATIVE_KEYWORDS = ["별로", "후회", "실망", "불편", "최악", "아쉽", "별루"];

export async function checkBrandReviews(auth, brandName, opts = {}) {
  const display = opts.displayPerQuery ?? 10;
  const queries = [`${brandName} 후기`, `${brandName} 리뷰`];
  const { blogs, failed } = await searchReviewQueries(auth, queries, { displayPerQuery: display });

  let posCount = 0;
  let negCount = 0;
  blogs.forEach((b) => {
    const text = `${b.title} ${b.description || ""}`;
    if (POSITIVE_KEYWORDS.some((k) => text.includes(k))) posCount++;
    if (NEGATIVE_KEYWORDS.some((k) => text.includes(k))) negCount++;
  });
  const totalSignal = posCount + negCount;
  const sentimentScore = totalSignal > 0 ? Math.round((posCount / totalSignal) * 100) : 50;

  // 적정 범위 판정 — 10~200 = "리뷰는 있는데 마케팅 약함" 영역
  let scaleSignal;
  if (blogs.length === 0) scaleSignal = "absent";
  else if (blogs.length < 10) scaleSignal = "weak";
  else if (blogs.length <= 200) scaleSignal = "sweetspot";
  else scaleSignal = "saturated";
  ...
  return {
    ...
    negCount,
    scaleSignal,
    sampleTitles: blogs.slice(0, 3).map((b) => b.title),
    failed,
  };
}
