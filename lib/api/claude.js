...
한국 블로그 후기 (선택적으로 US YouTube 리뷰 제목) 데이터를 받아서, 소비자 불만/아쉬운 점/개선 요청을 추출합니다.

# 분석 원칙
- 칭찬 일색의 협찬·홍보 리뷰는 무시하고 부정 의견/단점/아쉬움에 집중.
- 실제 리뷰 텍스트에서 인용 (quotes). 상상·추측 금지. 인용은 원문 형태를 거의 그대로.
- 인용에서 인명·블로거명·URL 은 제거.
- 같은 불만이 여러 리뷰에 나오면 frequency 에 명시.
...
# JSON 스키마
{
  "totalReviews": 0,                  // 분석한 리뷰 개수
  "negativeRatio": 0,                 // 0-100, 부정 의견 비율 추정
  "complaints": [
    {
      "issue": "불만 한 줄 (50자)",
      "severity": 0,                  // 0-100, 빈도 × 강도
      "frequency": "30개 중 N개 언급",
      "quotes": ["원문 인용 1 (80자 이내)", "원문 인용 2"],
      "productDirection": "이 불만을 해결하는 제품 개발 방향 (80자)",
      "recommendedSpec": "추천 스펙 (50자)"
    }
  ],
  "positives": ["자주 언급되는 장점 1 (40자)", "장점 2", "장점 3"],
  "insight": "분석 종합 인사이트 3-5줄 (200자 이내)"
}`;

function buildReviewUserPrompt({ categoryName, isSubstitute, substituteName, blogs, ytVideos }) {
  const lines = [];
  lines.push(`# 분석 대상`);
  if (isSubstitute) {
    lines.push(`- 신제품 카테고리: ${categoryName}`);
    lines.push(`- 분석할 대체재: "${substituteName}"`);
    lines.push(
      `- 한국에 신제품 직접 리뷰가 부족하여 대체재 리뷰로 기회를 도출합니다. 대체재의 불만이 곧 신제품이 해결할 기회입니다.`
    );
  } else {
  ...
  lines.push(``);
  lines.push(`# 네이버 블로그 리뷰 ${blogs.length}개`);
  blogs.forEach((b, i) => {
    lines.push(`${i + 1}. [${b.sourceQuery || "리뷰"}] ${b.title}`);
    if (b.description) lines.push(`   ${b.description.slice(0, 200)}`);
  });

  if (Array.isArray(ytVideos) && ytVideos.length) {
    lines.push(``);
    lines.push(`# US YouTube 리뷰 영상 제목 ${ytVideos.length}개 (참고용)`);
    ytVideos.forEach((v, i) => lines.push(`${i + 1}. ${v.title}`));
  }

  lines.push(``);
  if (isSubstitute) {
    lines.push(
      `이 대체재 리뷰에서 발견되는 불만이 곧 "${categoryName}" 신제품이 해결할 기회입니다. 그 관점에서 분석하여 JSON 으로 응답하세요.`
    );
  } else {
    lines.push(
      `위 리뷰들에서 "${categoryName}" 에 대한 소비자 불만을 추출하여 JSON 으로 응답하세요.`
    );
  }
  return lines.join("\n");
}
